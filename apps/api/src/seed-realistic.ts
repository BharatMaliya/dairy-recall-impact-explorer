...
const rawLots = farms.flatMap((farm) => Array.from({length: days}, (_, dayIndex) => ['AM', 'PM'].map((shift, shiftIndex) => ({id: `RML-${farm.id}-${day(dayIndex)}-${shift}`, name: `${shift === 'AM' ? 'Morning' : 'Evening'} collection · ${day(dayIndex)}`, shift, farmId: farm.id, centerId: farm.centerId, tankId: farm.tankId, vehicleId: farm.vehicleId, batchId: `PB-${day(dayIndex)}-${farm.tankId}`, collectionDate: day(dayIndex), collectedAt: `${day(dayIndex)}T${shiftIndex ? '16:30' : '05:30'}:00+05:30`, quantityLiters: 180 + ((dayIndex * 17 + shiftIndex * 31 + Number(farm.id.slice(-3))) % 240)}))));
...
async function seed() {
  ...
  await run('Daily raw-milk lots', `UNWIND $rows AS row MERGE (raw:RawMilkLot:Lot:Entity {id:row.id}) SET raw.name=row.name,raw.shift=row.shift,raw.collectionDate=row.collectionDate,raw.collectedAt=row.collectedAt,raw.quantityLiters=row.quantityLiters,raw.qualityStatus='RELEASED'`, rawLots);
